...
def main(config):    
    ...
    for sigma in sigma_nums:
        for trial in trial_nums:
            ...
            load_path = os.path.join('3_denoising','noisy_images',config['img_name'],'sigma_'+str(sigma)+'_trial_'+str(trial))
            # Create direcotry for saving results
            save_path = os.path.join('3_denoising','results',config['img_name'],'sigma_'+str(sigma)+'_trial_'+str(trial),'fastsolver')
            if not os.path.isdir(save_path):
                os.makedirs(save_path)

            # Load dataset
            gen_data = torch.Tensor(scipy.io.loadmat(os.path.join(load_path,'noisy_train_data.mat'))['noisy_data']).float().to(config['device'])
            ...
            if config['init'] == 'odct':
                ...
                load_path = os.path.join('3_denoising','noisy_images',config['img_name'],'sigma_'+str(sigma)+'_trial_'+str(trial))
                learnt_dict = torch.Tensor(scipy.io.loadmat(os.path.join(load_path,'init_dict.mat'))['initdict']).float().to(config['device'])
            ...
            np.save(os.path.join(save_path,'learnt_dict_orig.npy'),learnt_dict)
            scipy.io.savemat(os.path.join(save_path,'learnt_dict_orig.mat'),{'D': learnt_dict})
            # Normalize dictionary
            learnt_dict_norm = learnt_dict/np.linalg.norm(learnt_dict,axis=0)
            np.save(os.path.join(save_path,'learnt_dictionary_norm.npy'),learnt_dict_norm)
            scipy.io.savemat(os.path.join(save_path,'learnt_dict_norm.mat'),{'D': learnt_dict_norm})
            ...
            plt.savefig(os.path.join(save_path,'learnt_dictionary.png'),transparent=True,bbox_inches='tight')
            plt.close()
