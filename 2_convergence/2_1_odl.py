...
def main():
    data_load_path = '2_convergence/dataset/'
    num_trials = 50
    success_rate = 0.0
    num_atoms = 50 # k
    sparsity = 3 # s
    ...
    for trial in range(1,num_trials+1):
        ...
        save_path = '2_convergence/results/odl_trial_'+str(trial)+'/'
        if not os.path.isdir(save_path):
            os.makedirs(save_path)
        # Load generation dictionary and data
        gen_dictionary_norm = scipy.io.loadmat(data_load_path+'trial_'+str(trial)+\
                                            '/gen_dictionary.mat')['D']
        dict_size = gen_dictionary_norm.shape[1]
        gen_data = scipy.io.loadmat(data_load_path+'trial_'+str(trial)+'/gen_data.mat')['X']        
        ...
        np.save(save_path+'D_ODL',learnt_dictionary_norm)
        np.save(save_path+'Z_ODL',learnt_codes)
    ...
    np.save('2_convergence/results/odl_num_detected_atoms',num_detected_atoms)
    np.save('2_convergence/results/odl_time_consumed',time_consumed)
